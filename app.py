...
from flask import (Flask, render_template, request, redirect, url_for, flash,
                   session, jsonify, send_from_directory, abort, Response, send_file)
...
class PhysicalInventoryItem(db.Model):
    """Individual item counts in a physical inventory - DEA/NC DHHS Compliant"""
    id = db.Column(db.Integer, primary_key=True)
    ...
    # Count verification (who counted and witnessed)
    counted_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    counted_at = db.Column(db.DateTime)
    verified_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    verified_at = db.Column(db.DateTime)
    
    # Initial discrepancy notes (at time of count)
    discrepancy_notes = db.Column(db.Text)
    
    # === DEA/NC DHHS COMPLIANT INVESTIGATION FIELDS ===
    
    # Investigation Status: discovered, under_investigation, pending_review, resolved, referred_to_dea
    investigation_status = db.Column(db.String(30), default='discovered')
    
    # Investigation Category per DEA Guidelines
    # documentation_error, counting_error, theft_diversion, breakage_spillage, 
    # system_error, variance_tolerance, expired_destroyed, other
    investigation_category = db.Column(db.String(50))
    
    # Investigation tracking
    investigation_started_at = db.Column(db.DateTime)
    investigation_started_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    ...
    # DEA Form 106 requirement (theft/diversion cases)
    requires_dea_report = db.Column(db.Boolean, default=False)
    dea_report_filed = db.Column(db.Boolean, default=False)
    dea_report_number = db.Column(db.String(50))
    dea_report_date = db.Column(db.Date)
    ...
    inventory_adjusted = db.Column(db.Boolean, default=False)
    adjustment_witness_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    adjustment_transaction_id = db.Column(db.Integer)
    
    # Supervisor review (required for Schedule II or significant discrepancies)
    supervisor_reviewed = db.Column(db.Boolean, default=False)
    supervisor_reviewed_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    supervisor_reviewed_at = db.Column(db.DateTime)
    ...
    counter = db.relationship('User', foreign_keys=[counted_by])
    verifier = db.relationship('User', foreign_keys=[verified_by])
    investigator = db.relationship('User', foreign_keys=[investigation_started_by])
    resolver = db.relationship('User', foreign_keys=[resolved_by])
    adjustment_witness = db.relationship('User', foreign_keys=[adjustment_witness_id])
    ...
    @property
    def is_schedule_ii(self):
        """Check if this is a Schedule II controlled substance"""
        if self.inventory_item:
            return self.inventory_item.medication.schedule == 'II'
        elif self.patient_medication:
            return self.patient_medication.medication.schedule == 'II'
        return False
    
    @property
    def requires_supervisor_review(self):
        """Schedule II or significant discrepancies require supervisor review"""
        if self.is_schedule_ii:
            return True
        # Significant discrepancy = shortage > 5% or any shortage > 10 units
        if self.discrepancy < 0:
            if self.expected_quantity > 0:
                pct = abs(self.discrepancy) / self.expected_quantity
                if pct > 0.05:
                    return True
            if abs(self.discrepancy) > 10:
                return True
        return False
    
    @property
    def days_since_discovery(self):
        """Days since discrepancy was discovered (for compliance tracking)"""
        if self.counted_at:
            return (get_local_now() - self.counted_at).days
        return 0
...
def receive_inventory():
    if request.method == 'POST':
        ...
        # Check if there's an existing active inventory item with the same medication and lot number
        existing_item = None
        if lot_number:
        ...
        if existing_item:
            ...
            old_qty = existing_item.current_quantity
            existing_item.quantity_received += quantity
            existing_item.current_quantity += quantity
            
            # Update expiration date if provided and different
            if expiration_date and expiration_date != existing_item.expiration_date:
                existing_item.expiration_date = expiration_date
            
            # Update storage location if provided
            if request.form.get('storage_location'):
                existing_item.storage_location = request.form.get('storage_location')
            
            # Append to notes if provided
            new_notes = request.form.get('notes', '').strip()
            if new_notes:
                if existing_item.notes:
                    existing_item.notes += f"\n[{get_current_time().strftime('%Y-%m-%d')}] {new_notes}"
                else:
                    existing_item.notes = new_notes
            
            db.session.commit()
            
            log_audit('receive_inventory', 'inventory_item', existing_item.id,
                     f'Added {quantity} {unit_count} to existing lot of {med.name}, Lot: {lot_number} (was {old_qty}, now {existing_item.current_quantity})')
            
            flash(f'Inventory combined with existing lot. Added {quantity} {unit_count} to {med.name} (Lot: {lot_number}). New total: {existing_item.current_quantity} {unit_count}.', 'success')
            ...
            flash(f'Inventory received successfully. {quantity} {unit_count} of {med.name} added.', 'success')
            return redirect(url_for('inventory'))
...
def dispense():
    if request.method == 'POST':
        ...
        # Parse dispense date/time (optional - defaults to now)
        dispense_datetime = get_local_now()
        dispense_date_str = request.form.get('dispense_date', '').strip()
        dispense_time_str = request.form.get('dispense_time', '').strip()
        
        if dispense_date_str:
            try:
                if dispense_time_str:
                    # Both date and time provided
                    dispense_datetime = datetime.strptime(f"{dispense_date_str} {dispense_time_str}", '%Y-%m-%d %H:%M')
                else:
                    # Only date provided - use current time
                    current_time = get_local_now().strftime('%H:%M')
                    dispense_datetime = datetime.strptime(f"{dispense_date_str} {current_time}", '%Y-%m-%d %H:%M')
            except ValueError:
        ...
        transaction = Transaction(
            ...
            performed_by=session['user_id'],
            performed_at=dispense_datetime,
            notes=request.form.get('notes')
        )
    ...
    return render_template('dispense.html', inventory_items=inventory_items, providers=providers, patients=patients, now=get_local_now())
...
def waste():
    if request.method == 'POST':
        ...
        # Parse waste date/time (optional - defaults to now)
        waste_datetime = get_local_now()
        waste_date_str = request.form.get('waste_date', '').strip()
        waste_time_str = request.form.get('waste_time', '').strip()
        
        if waste_date_str:
            try:
                if waste_time_str:
                    # Both date and time provided
                    waste_datetime = datetime.strptime(f"{waste_date_str} {waste_time_str}", '%Y-%m-%d %H:%M')
                else:
                    # Only date provided - use current time
                    current_time = get_local_now().strftime('%H:%M')
                    waste_datetime = datetime.strptime(f"{waste_date_str} {current_time}", '%Y-%m-%d %H:%M')
            except ValueError:
                # Fall back to now if parsing fails
                waste_datetime = get_local_now()
        ...
        transaction = Transaction(
            ...
            witness_signature_time=waste_datetime,
            performed_by=session['user_id'],
            performed_at=waste_datetime,
            notes=request.form.get('notes')
        )
    ...
    return render_template('waste.html', inventory_items=items, now=get_local_now())
...
@app.route('/discrepancy/<int:id>/resolve', methods=['GET', 'POST'])
@login_required
def resolve_discrepancy(id):
    """
    DEA/NC DHHS Compliant Discrepancy Resolution Workflow
    
    Investigation Categories per DEA Guidelines:
    - documentation_error: Paperwork or recording mistakes
    - counting_error: Human error during physical count
    - theft_diversion: Suspected theft or diversion (triggers DEA Form 106)
    - breakage_spillage: Physical loss due to breakage/spillage
    - system_error: Computer/system calculation errors
    - variance_tolerance: Minor variance within acceptable limits
    - expired_destroyed: Discrepancy due to expired/destroyed stock
    - other: Other causes requiring explanation
    """
    # Get list of potential witnesses (active users except current user)
    witnesses = User.query.filter(
        User.is_active == True,
    ...
    # Get supervisors for review (admins and providers)
    ...
    if not item:
        # Fall back to legacy DailyCount
        count = DailyCount.query.get_or_404(id)
        # Handle legacy daily count resolution (simplified)
        if request.method == 'POST':
            resolution_type = request.form.get('investigation_category', 'other')
    ...
    if item.inventory_item:
        medication = item.inventory_item.medication
        lot_number = item.inventory_item.lot_number
    elif item.patient_medication:
        medication = item.patient_medication.medication
        lot_number = item.patient_medication.lot_number
    else:
        flash('Invalid discrepancy record.', 'danger')
    ...
    if request.method == 'POST':
        ...
        if action == 'start_investigation':
            ...
            item.investigation_started_at = get_local_now()
            item.investigation_started_by = session['user_id']
            db.session.commit()
            
            log_audit('start_investigation', 'physical_inventory_item', item.id,
                     f'Investigation started for {medication.name} discrepancy')
            flash('Investigation started. Document findings and resolve.', 'info')
            return redirect(url_for('resolve_discrepancy', id=id))
        
        # === SUBMIT RESOLUTION ===
        elif action == 'resolve':
            # Get form data
            investigation_category = request.form.get('investigation_category')
            ...
            # Validation
            if not investigation_category:
                flash('Investigation category is required.', 'danger')
                return render_template('resolve_discrepancy.html', count=item,
                                     is_physical_inventory=True, witnesses=witnesses,
                                     supervisors=supervisors, medication=medication)
            
            if not root_cause:
                flash('Root cause analysis is required.', 'danger')
                return render_template('resolve_discrepancy.html', count=item,
                                     is_physical_inventory=True, witnesses=witnesses,
                                     supervisors=supervisors, medication=medication)
            
            # Theft/diversion requires additional handling
            if investigation_category == 'theft_diversion':
                item.requires_dea_report = True
                flash('⚠️ IMPORTANT: This discrepancy requires DEA Form 106 filing within 24 hours of discovery.', 'warning')
            ...
            if adjust_inventory:
                ...
                witness = User.query.get(witness_id)
                if not witness or not witness.check_password(witness_password):
                    flash('Invalid witness credentials.', 'danger')
                    return render_template('resolve_discrepancy.html', count=item,
                                         is_physical_inventory=True, witnesses=witnesses,
                                         supervisors=supervisors, medication=medication)
            ...
            item.investigation_category = investigation_category
            item.root_cause = root_cause
            item.corrective_action = corrective_action
            item.resolution_notes = resolution_notes
            item.resolved_by = session['user_id']
            item.resolved_at = get_local_now()
            
            # If supervisor review required, mark as pending review
            if item.requires_supervisor_review or investigation_category == 'theft_diversion':
                item.investigation_status = 'pending_review'
                item.discrepancy_resolved = False
            else:
            ...
            if adjust_inventory:
                item.inventory_adjusted = True
                item.adjustment_witness_id = int(witness_id)
                ...
                if item.inventory_item:
                    ...
                    transaction = Transaction(
                        ...
                        adjustment_reason=f"Discrepancy Resolution [{investigation_category.upper()}]: {root_cause}",
                        performed_by=session['user_id'],
                        witness_id=int(witness_id)
                    )
                    db.session.add(transaction)
                    item.adjustment_transaction_id = transaction.id
                    
                    log_audit('adjust_inventory', 'inventory_item', inv_item.id,
                             f'Discrepancy adjustment: {balance_before} → {inv_item.current_quantity} '
                             f'(Category: {investigation_category}, Witness: {witness.full_name})')
            ...
            log_audit('resolve_discrepancy', 'physical_inventory_item', item.id,
                     f'Category: {investigation_category}, Root Cause: {root_cause[:100]}')
            
            if item.investigation_status == 'pending_review':
                flash('Resolution submitted. Pending supervisor review.', 'info')
            else:
                flash('Discrepancy resolved successfully.', 'success')
        ...
        # === SUPERVISOR REVIEW ===
        elif action == 'supervisor_review':
            # Only admins/providers can do supervisor review
            current_user = User.query.get(session['user_id'])
            if current_user.role not in ['admin', 'provider']:
                flash('Only supervisors can approve discrepancy resolutions.', 'danger')
                return redirect(url_for('resolve_discrepancy', id=id))
            
            approval_status = request.form.get('approval_status')
            supervisor_notes = request.form.get('supervisor_notes', '').strip()
            
            if approval_status == 'approve':
                item.supervisor_reviewed = True
                item.supervisor_reviewed_by = session['user_id']
                item.supervisor_reviewed_at = get_local_now()
                item.supervisor_notes = supervisor_notes
                item.investigation_status = 'resolved'
                item.discrepancy_resolved = True
                
                db.session.commit()
                
                log_audit('supervisor_approve_discrepancy', 'physical_inventory_item', item.id,
                         f'Supervisor approved resolution. Notes: {supervisor_notes[:100] if supervisor_notes else "None"}')
                
                flash('Resolution approved. Discrepancy closed.', 'success')
            
            elif approval_status == 'reject':
                item.supervisor_notes = f"REJECTED: {supervisor_notes}"
                item.investigation_status = 'under_investigation'
                
                db.session.commit()
                
                log_audit('supervisor_reject_discrepancy', 'physical_inventory_item', item.id,
                         f'Supervisor rejected resolution. Reason: {supervisor_notes}')
                
                flash('Resolution rejected. Requires additional investigation.', 'warning')
            
            return redirect(url_for('discrepancy_report'))
        
        # === MARK DEA FORM 106 FILED ===
        elif action == 'mark_dea_filed':
            dea_report_number = request.form.get('dea_report_number', '').strip()
            if not dea_report_number:
                flash('DEA Form 106 report number is required.', 'danger')
                return redirect(url_for('resolve_discrepancy', id=id))
            
            item.dea_report_filed = True
            item.dea_report_number = dea_report_number
            item.dea_report_date = get_current_date()
            
            db.session.commit()
            
            log_audit('dea_form_106_filed', 'physical_inventory_item', item.id,
                     f'DEA Form 106 filed. Report #: {dea_report_number}')
            
            flash(f'DEA Form 106 filing recorded. Report #: {dea_report_number}', 'success')
            return redirect(url_for('resolve_discrepancy', id=id))
...
@login_required
def resolve_discrepancy_legacy(id):
    return redirect(url_for('resolve_discrepancy', id=id))
...
def dispensing_log():
    ...
    # Build query for regular dispense transactions (exclude voided)
    ...
    for tx in transactions:
        unified_records.append({
            ...
            'expiration_date': tx.inventory_item.expiration_date if tx.inventory_item else None,
    ...
    for log in patient_logs:
        ...
        unified_records.append({
            ...
            'expiration_date': pm.expiration_date if pm else None,
...
def waste_log():
    ...
    # Build query - ONLY waste transactions (exclude voided)
    query = Transaction.query.filter(
        Transaction.transaction_type == 'waste',
        Transaction.is_voided == False
    ).join(InventoryItem).join(Medication)
...
def receive_form222(id):
    ...
    if request.method == 'POST':
        ...
        for line_item in form222.line_items:
            ...
            lot_number = request.form.get(f'lot_number_{line_item.id}', '').strip()
            ...
            if qty_received:
                ...
                if qty > 0:
                    ...
                    if lot_number:
                        existing_item = InventoryItem.query.filter(
                            InventoryItem.medication_id == line_item.medication_id,
                            InventoryItem.lot_number == lot_number,
                            InventoryItem.is_active == True
                        ).first()
                    ...
                    if existing_item:
                        # Combine with existing item
                        old_qty = existing_item.current_quantity
                        existing_item.quantity_received += qty
                        existing_item.current_quantity += qty
                        
                        # Update expiration date if provided and different
                        if exp_date and exp_date != existing_item.expiration_date:
                            existing_item.expiration_date = exp_date
                        
                        # Append Form 222 reference to notes
                        form222_note = f"[{get_current_time().strftime('%Y-%m-%d')}] Added {qty} via Form 222 #{form222.form_number}"
                        if existing_item.notes:
                            existing_item.notes += f"\n{form222_note}"
                        else:
                            existing_item.notes = form222_note
                        
                        db.session.flush()
                        line_item.inventory_item_id = existing_item.id
                        
                        # Log the receipt
                        log_audit('receive_form222', 'inventory_item', existing_item.id,
                                 f'Added {qty} to existing lot of {line_item.medication.name} via Form 222 #{form222.form_number} (was {old_qty}, now {existing_item.current_quantity})')
                    else:
                        # Create new inventory item
...
def adjust_from_physical_inventory(id):
    ...
    # Require a reason for the adjustment
    reason = request.form.get('reason', '').strip()
    if not reason:
        flash('A reason is required for inventory adjustments.', 'danger')
        return redirect(url_for('physical_inventory_report', id=id))
    ...
    transaction = Transaction(
        ...
        adjustment_reason=f'Physical inventory adjustment: {reason}',
        performed_by=session['user_id']
    )
    ...
    count_item.resolution_notes = reason
    
    db.session.add(transaction)
    db.session.commit()
    
    log_audit('physical_inventory_adjust', 'inventory_item', inv_item.id,
             f'Adjusted quantity from {old_qty} to {new_qty}. Reason: {reason}')
...
def settings():
    ...
    settings_data = {
        'practice_name': SystemSetting.get('practice_name', 'Magnolia Health PLLC'),
        'practice_address': SystemSetting.get('practice_address', ''),
        'practice_phone': SystemSetting.get('practice_phone', ''),
        'practice_fax': SystemSetting.get('practice_fax', ''),
        'low_inventory_threshold': int(SystemSetting.get('low_inventory_threshold', '10')),
        'expiration_warning_days': int(SystemSetting.get('expiration_warning_days', '90')),
        'require_daily_counts': SystemSetting.get('require_daily_counts', 'true') == 'true',
        'require_dual_verification': SystemSetting.get('require_dual_verification', 'true') == 'true'
    }
...
def update_practice_info():
    ...
    SystemSetting.set('practice_name', request.form.get('practice_name', '').strip())
    SystemSetting.set('practice_address', request.form.get('practice_address', '').strip())
    SystemSetting.set('practice_phone', request.form.get('practice_phone', '').strip())
    SystemSetting.set('practice_fax', request.form.get('practice_fax', '').strip())
    
    log_audit('update_settings', 'system', None, 'Updated practice information')
...
def update_system_settings():
    ...
    SystemSetting.set('low_inventory_threshold', request.form.get('low_inventory_threshold', '10'))
    SystemSetting.set('expiration_warning_days', request.form.get('expiration_warning_days', '90'))
    SystemSetting.set('require_daily_counts', 'true' if request.form.get('require_daily_counts') else 'false')
    SystemSetting.set('require_dual_verification', 'true' if request.form.get('require_dual_verification') else 'false')
    
    log_audit('update_settings', 'system', None, 'Updated system settings')
...
def export_report():
    ...
    if report_type == 'transactions':
        ...
        headers = ['Date', 'Time', 'Type', 'Medication', 'Strength', 'Schedule', 'Lot #', 'Qty', 'Balance', 'Patient', 'User', 'Witness']
        data = []
        for t in query.order_by(Transaction.performed_at.desc()).limit(500).all():
            try:
                unit = t.inventory_item.unit_count or t.inventory_item.medication.unit or 'units'
                qty = t.quantity if t.quantity is not None else 0
                balance = t.balance_after if t.balance_after is not None else 0
                # Format numbers - remove trailing zeros
                qty_str = f"{qty:g}" if isinstance(qty, float) else str(qty)
                balance_str = f"{balance:g}" if isinstance(balance, float) else str(balance)
                data.append([
                    t.performed_at.strftime('%m/%d/%Y'),
                    t.performed_at.strftime('%I:%M %p'),
                    t.transaction_type.title() if t.transaction_type else '-',
                    t.inventory_item.medication.name if t.inventory_item and t.inventory_item.medication else 'Unknown',
                    t.inventory_item.medication.strength or '-' if t.inventory_item and t.inventory_item.medication else '-',
                    t.inventory_item.medication.schedule if t.inventory_item and t.inventory_item.medication else '-',
                    t.inventory_item.lot_number or 'N/A' if t.inventory_item else 'N/A',
                    f"{qty_str} {unit}",
                    f"{balance_str} {unit}",
                    t.patient_name or '-',
        ...
        # Regular dispensing transactions (exclude voided)
        query = Transaction.query.filter(
            Transaction.transaction_type == 'dispense',
            Transaction.is_voided == False
        ).join(InventoryItem).join(Medication)
        ...
        for t in query.order_by(Transaction.performed_at.desc()).all():
            ...
            unified_records.append({
                ...
                'lot_number': t.inventory_item.lot_number or '-',
                'expiration': t.inventory_item.expiration_date.strftime('%m/%d/%Y') if t.inventory_item.expiration_date else '-',
        ...
        for log in patient_query.order_by(PatientMedicationLog.administration_date.desc()).all():
            ...
            unified_records.append({
                ...
                'schedule': pm.medication.schedule if pm.medication else '',
                'lot_number': pm.lot_number or '-',
                'expiration': pm.expiration_date.strftime('%m/%d/%Y') if pm.expiration_date else '-',
        ...
        headers = ['Date', 'Time', 'Medication', 'Strength', 'Schedule', 'Lot #', 'Exp', 'Qty', 'Patient', 'Prescriber', 'Dispensed By', 'Source']
        data = []
        ...
        headers = ['Date', 'Time', 'Medication', 'Strength', 'Schedule', 'Lot #', 'Qty', 'Reason', 'Wasted By', 'Witness']
        data = []
        for t in query.order_by(Transaction.performed_at.desc()).all():
            unit = t.inventory_item.unit_count or t.inventory_item.medication.unit or 'units'
            data.append([
                ...
                t.inventory_item.lot_number or '-',
                f"{t.quantity} {unit}",
                t.waste_reason or '-',
                t.performer.full_name if t.performer else 'Unknown',
                t.witness.full_name if t.witness else '-'
        ...
        data = []
        for i in query.order_by(InventoryItem.date_received.desc()).limit(500).all():
            # supplier is a string field, not a relationship
            supplier_name = i.supplier or '-'
            data.append([
        ...
        headers = ['Date', 'Medication', 'Strength', 'Schedule', 'Lot #', 'Expected', 'Actual', 'Variance', 'Counted By', 'Witness']
        data = []
        for c in query.order_by(DailyCount.count_date.desc(), DailyCount.counted_at.desc()).all():
            data.append([
                ...
                c.verifier.full_name if c.verifier else '-'
            ])
        title = 'Daily Count History Report'
    
    elif report_type == 'discrepancies':
        # Query PhysicalInventoryItem for discrepancies (matches the report view)
        query = PhysicalInventoryItem.query.join(
            PhysicalInventory, PhysicalInventoryItem.physical_inventory_id == PhysicalInventory.id
        ).filter(
            PhysicalInventoryItem.actual_quantity.isnot(None)
        ...
        if start_date and start_date.strip():
            try:
                start = datetime.strptime(start_date, '%Y-%m-%d').date()
                discrepancy_items = [d for d in discrepancy_items if d.physical_inventory.inventory_date >= start]
            except ValueError:
                pass
        ...
        if end_date and end_date.strip():
            try:
                ...
                discrepancy_items = [d for d in discrepancy_items if d.physical_inventory.inventory_date <= end]
            except ValueError:
                pass
        
        # Sort by date descending
        discrepancy_items.sort(key=lambda x: x.physical_inventory.inventory_date, reverse=True)
        
        # Generate compact single-page PDF for discrepancies
        from reportlab.lib import colors
        from reportlab.lib.pagesizes import letter
        from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
        from reportlab.lib.units import inch
        ...
        # Compact styles
        title_style = ParagraphStyle('Title', parent=styles['Heading1'], fontSize=12, alignment=TA_CENTER, spaceAfter=4)
        subtitle_style = ParagraphStyle('Subtitle', parent=styles['Normal'], fontSize=8, alignment=TA_CENTER, textColor=colors.grey, spaceAfter=8)
        cell_style = ParagraphStyle('Cell', parent=styles['Normal'], fontSize=7, leading=9)
        cell_style_small = ParagraphStyle('CellSmall', parent=styles['Normal'], fontSize=6, leading=8, textColor=colors.grey)
        
        # Header
        elements.append(Paragraph("CONTROLLED SUBSTANCE DISCREPANCY REPORT", title_style))
        elements.append(Paragraph(f"Magnolia Health PLLC | Generated: {get_current_time().strftime('%m/%d/%Y %I:%M %p')} | "
                                  f"Date Range: {start_date or 'All'} to {end_date or 'Present'}", subtitle_style))
        
        # Summary line
        pending_count = len([d for d in discrepancy_items if not d.discrepancy_resolved])
        resolved_count = len([d for d in discrepancy_items if d.discrepancy_resolved])
        summary_text = f"Total: {len(discrepancy_items)} | Pending: {pending_count} | Resolved: {resolved_count}"
        elements.append(Paragraph(f"<b>{summary_text}</b>", ParagraphStyle('Summary', fontSize=8, alignment=TA_CENTER, spaceAfter=8)))
        
        if not discrepancy_items:
            elements.append(Paragraph("No discrepancies found for the selected date range.", cell_style))
        else:
            # Build compact table
            table_data = [[
                Paragraph('<b>Date</b>', cell_style),
                Paragraph('<b>Medication</b>', cell_style),
                Paragraph('<b>Sch</b>', cell_style),
                Paragraph('<b>Lot #</b>', cell_style),
                Paragraph('<b>Expected</b>', cell_style),
                Paragraph('<b>Actual</b>', cell_style),
                Paragraph('<b>Variance</b>', cell_style),
                Paragraph('<b>Category</b>', cell_style),
                Paragraph('<b>Status</b>', cell_style),
                Paragraph('<b>Resolution</b>', cell_style),
            ]]
            
            for d in discrepancy_items:
                # Get medication info
                if d.inventory_item:
                    med_name = d.inventory_item.medication.name
                    schedule = d.inventory_item.medication.schedule
                    lot = d.inventory_item.lot_number or '-'
                    unit = d.inventory_item.unit_count or d.inventory_item.medication.unit or 'units'
                elif d.patient_medication:
                    med_name = d.patient_medication.medication.name
                    schedule = d.patient_medication.medication.schedule
                    lot = d.patient_medication.lot_number or '-'
                    unit = d.patient_medication.unit or 'units'
                else:
                    continue
                
                # Truncate long names
                if len(med_name) > 18:
                    med_name = med_name[:16] + '...'
                if len(lot) > 12:
                    lot = lot[:10] + '...'
                
                variance_text = f"{d.discrepancy:+g}"
                variance_type = "SHORT" if d.discrepancy < 0 else "OVER"
                
                # Status
                if d.discrepancy_resolved:
                    status = "Resolved"
                elif hasattr(d, 'investigation_status') and d.investigation_status == 'pending_review':
                    status = "Review"
                elif hasattr(d, 'investigation_status') and d.investigation_status == 'under_investigation':
                    status = "Invest."
                else:
                    status = "New"
                ...
                # Category
                category = '-'
                if hasattr(d, 'investigation_category') and d.investigation_category:
                    cat = d.investigation_category.replace('_', ' ').title()
                    if len(cat) > 10:
                        cat = cat[:8] + '..'
                    category = cat
                ...
                resolution = '-'
                if d.discrepancy_resolved and d.resolution_notes:
                    res = d.resolution_notes
                    if len(res) > 35:
                        res = res[:33] + '...'
                    resolution = res
                elif hasattr(d, 'root_cause') and d.root_cause:
                    res = d.root_cause
                    if len(res) > 35:
                        res = res[:33] + '...'
                    resolution = res
                ...
                table_data.append([
                    Paragraph(d.physical_inventory.inventory_date.strftime('%m/%d/%y'), cell_style),
                    Paragraph(med_name, cell_style),
                    Paragraph(schedule, cell_style),
                    Paragraph(lot, cell_style_small),
                    Paragraph(f"{d.expected_quantity:g}", cell_style),
                    Paragraph(f"{d.actual_quantity:g}", cell_style),
                    Paragraph(f"<b>{variance_text}</b>", cell_style),
                    Paragraph(category, cell_style_small),
                    Paragraph(status, cell_style),
                    Paragraph(resolution, cell_style_small),
                ])
            
            # Create table with compact column widths
            col_widths = [0.55*inch, 1.1*inch, 0.3*inch, 0.7*inch, 0.5*inch, 0.5*inch, 0.55*inch, 0.65*inch, 0.5*inch, 2.15*inch]
        ...
        elements.append(Paragraph("Generated by CS Inventory System | DEA 21 CFR 1304 Compliant | Magnolia Health PLLC", footer_style))
        ...
        return send_file(
            ...
            mimetype='application/pdf',
            as_attachment=True,
            download_name=f"discrepancy_report_{get_current_time().strftime('%Y%m%d_%H%M')}.pdf"
        )
    
    elif report_type == 'audit_log':
        # Get action filter if provided
        ...
        headers = ['Date', 'Time', 'User', 'Action', 'Details', 'IP Address']
        data = []
        for log in query.order_by(AuditLog.timestamp.desc()).limit(500).all():
            data.append([
                log.timestamp.strftime('%m/%d/%Y'),
    ...
    if format_type == 'pdf':
        try:
            ...
            # Sanitize data for PDF - replace special characters
            def sanitize_for_pdf(value):
                if value is None:
                    return '-'
                s = str(value)
                # Replace characters that can cause issues
                s = s.replace('&', '&amp;')
                s = s.replace('<', '&lt;')
                s = s.replace('>', '&gt;')
                return s
            ...
            sanitized_data = []
            for row in data:
                sanitized_row = [sanitize_for_pdf(cell) for cell in row]
                sanitized_data.append(sanitized_row)
            
            buffer = BytesIO()
            
            # Use landscape for reports with many columns (6 or more)
            ...
            elements.append(Paragraph(sanitize_for_pdf(title), title_style))
            ...
            # Calculate column widths - give more space to certain columns
            ...
            for i, h in enumerate(headers):
                h_lower = h.lower()
                if h_lower in ['medication', 'drug name', 'drug']:
                    col_indices['medication'] = i
                elif h_lower == 'patient':
                    col_indices['patient'] = i
                elif h_lower == 'supplier':
                    col_indices['supplier'] = i
                elif h_lower == 'strength':
                    col_indices['strength'] = i
                elif h_lower == 'schedule':
                    col_indices['schedule'] = i
                elif h_lower == 'details':
                    col_indices['details'] = i
                elif h_lower == 'notes':
                    col_indices['notes'] = i
                elif h_lower == 'resolved':
                    col_indices['resolved'] = i
                elif h_lower == 'variance':
                    col_indices['variance'] = i
                elif h_lower == 'witness':
                    col_indices['witness'] = i
                elif h_lower == 'counted by':
                    col_indices['counted_by'] = i
                elif h_lower == 'lot #':
                    col_indices['lot'] = i
                elif h_lower == 'reason':
                    col_indices['reason'] = i
                elif h_lower == 'date':
                    col_indices['date'] = i
                elif h_lower == 'time':
                    col_indices['time'] = i
                ...
                elif h_lower == 'ip address':
                    col_indices['ip_address'] = i
                elif h_lower == 'user':
                    col_indices['user'] = i
                elif h_lower == 'exp':
                    col_indices['exp'] = i
                    ...
                    col_indices['qty'] = i
                elif h_lower == 'prescriber':
                    col_indices['prescriber'] = i
                elif h_lower == 'dispensed by':
                    col_indices['dispensed_by'] = i
            
            # Calculate widths with multipliers
            # Base calculation: sum of all multipliers
            multipliers = [1.0] * num_cols
            
            # Medication gets more space
            if 'medication' in col_indices:
                multipliers[col_indices['medication']] = 1.6
            
            # Patient gets more space
            if 'patient' in col_indices:
                multipliers[col_indices['patient']] = 1.3
            
            # Supplier gets more space
            if 'supplier' in col_indices:
                multipliers[col_indices['supplier']] = 1.5
            
            # Details gets more space (for audit log)
            if 'details' in col_indices:
                multipliers[col_indices['details']] = 3.0
            
            # Notes gets more space
            if 'notes' in col_indices:
                multipliers[col_indices['notes']] = 2.5
            
            # Strength and Schedule get less space (but not too narrow)
            if 'strength' in col_indices:
                multipliers[col_indices['strength']] = 0.7
            if 'schedule' in col_indices:
                multipliers[col_indices['schedule']] = 0.65
            
            # Resolved is just Yes/No - very narrow
            if 'resolved' in col_indices:
                multipliers[col_indices['resolved']] = 0.5
            
            # Variance can be narrower
            if 'variance' in col_indices:
                multipliers[col_indices['variance']] = 0.7
            
            # Lot # - moderate width
            if 'lot' in col_indices:
                multipliers[col_indices['lot']] = 0.9
            
            # Reason gets more space
            if 'reason' in col_indices:
                multipliers[col_indices['reason']] = 1.4
            
            # Date column - narrow
            if 'date' in col_indices:
                multipliers[col_indices['date']] = 0.7
            
            # Time column - narrow
            if 'time' in col_indices:
                multipliers[col_indices['time']] = 0.6
            
            # Action column - moderate
            if 'action' in col_indices:
                multipliers[col_indices['action']] = 1.2
            
            # IP Address column - narrow
            if 'ip_address' in col_indices:
                multipliers[col_indices['ip_address']] = 0.7
            
            # User column - moderate
            if 'user' in col_indices:
                multipliers[col_indices['user']] = 0.9
            
            # Exp column - narrow
            if 'exp' in col_indices:
                multipliers[col_indices['exp']] = 0.7
            
            # Source column - narrow
            if 'source' in col_indices:
                multipliers[col_indices['source']] = 0.8
            
            # Qty column - narrow
            if 'qty' in col_indices:
                multipliers[col_indices['qty']] = 0.6
            
            # Prescriber column - moderate
            if 'prescriber' in col_indices:
                multipliers[col_indices['prescriber']] = 0.9
            
            # Dispensed By column - moderate
            if 'dispensed_by' in col_indices:
                multipliers[col_indices['dispensed_by']] = 0.9
            
            # Calculate actual widths
            total_multiplier = sum(multipliers)
            base_width = available_width / total_multiplier
            col_widths = [base_width * m for m in multipliers]
            
            # For reports with Notes or long text columns, wrap text using Paragraph
            cell_style = ParagraphStyle('Cell', parent=styles['Normal'], fontSize=7, leading=8)
            wrap_columns = []
            if 'notes' in col_indices:
                wrap_columns.append(col_indices['notes'])
            if 'medication' in col_indices:
                wrap_columns.append(col_indices['medication'])
            if 'details' in col_indices:
                wrap_columns.append(col_indices['details'])
            if 'action' in col_indices:
                wrap_columns.append(col_indices['action'])
            if 'reason' in col_indices:
                wrap_columns.append(col_indices['reason'])
            ...
            if wrap_columns:
                ...
                for row_idx, row in enumerate(table_data):
                    ...
                    for col_idx in wrap_columns:
                        if len(new_row) > col_idx and row_idx > 0:  # Skip header row
                            new_row[col_idx] = Paragraph(str(new_row[col_idx]), cell_style)
            ...
            elements.append(Paragraph(f"Total Records: {len(sanitized_data)} | Controlled Substances Inventory System", footer_style))
...
with app.app_context():
    try:
        ...
        if 'physical_inventory_item' in existing_tables:
            ...
            # === DEA/NC DHHS COMPLIANT INVESTIGATION FIELDS ===
            dea_columns = [
                ('investigation_status', "VARCHAR(30) DEFAULT 'discovered'"),
                ('investigation_category', 'VARCHAR(50)'),
                ('investigation_started_at', 'TIMESTAMP'),
                ('investigation_started_by', 'INTEGER'),
                ('root_cause', 'TEXT'),
                ('corrective_action', 'TEXT'),
                ('requires_dea_report', 'BOOLEAN DEFAULT FALSE'),
                ('dea_report_filed', 'BOOLEAN DEFAULT FALSE'),
                ('dea_report_number', 'VARCHAR(50)'),
                ('dea_report_date', 'DATE'),
                ('resolved_by', 'INTEGER'),
                ('resolved_at', 'TIMESTAMP'),
                ('inventory_adjusted', 'BOOLEAN DEFAULT FALSE'),
                ('adjustment_witness_id', 'INTEGER'),
            ...
            for col_name, col_type in dea_columns:
                if col_name not in pii_columns:
                    try:
                        db.session.execute(text(f'ALTER TABLE physical_inventory_item ADD COLUMN {col_name} {col_type}'))
                        db.session.commit()
                        print(f"Added {col_name} column to physical_inventory_item table")
                    except Exception as e:
                        db.session.rollback()
                        pass
